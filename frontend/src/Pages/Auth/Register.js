...
import { registerAPI, adminRegister } from "../../utils/ApiRequest";
...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const apiEndpoint = role === "admin" ? adminRegister : registerAPI;
    ...
    try {
      const { data } = await axios.post(apiEndpoint, {
        ...
        email,
        password,
        role, // sending role to the backend
      });
  
      if (data.success === true) {
        delete data.user?.password;
        localStorage.setItem("user", JSON.stringify(data.user || data.admin)); // handle both user/admin
    ...
    <div style={{ position: 'relative', overflow: 'hidden' }}>
      ...
      <Row>
        <h1 className="text-center">
          <SavingsIcon sx={{ fontSize: 40, color: "white"}}  className="text-center" />
        </h1>
